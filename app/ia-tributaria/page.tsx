...
import { ArrowLeft, Send, AlertTriangle, CheckCircle, Lightbulb, Shield } from "lucide-react";
...
export default function IATributaria() {
  ...
  const enviarMensagem = (texto: string) => {
    ...
    setCarregando(true);
    setTimeout(() => {
      const resposta = respostas[texto] || "Ótima questão tributária! Para uma análise precisa, recomendo combinar este planejamento com seu contador. Posso detalhar alguma estratégia específica de redução de impostos dentro da legalidade?";
      ...
      <div className="rounded-2xl overflow-hidden" style={{background: "rgba(10,22,40,0.8)", border: "1px solid rgba(167,139,250,0.2)"}}>
        ...
        <div className="px-6 pb-4">
          ...
          <div className="flex gap-3">
            <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && enviarMensagem(input)} placeholder="Pergunte sobre impostos e planejamento tributário..." className="flex-1 px-4 py-3 rounded-xl focus:outline-none text-sm" style={{background: "rgba(255,255,255,0.04)", border: "1px solid rgba(167,139,250,0.2)", color: "#c8d8f0"}}/>
