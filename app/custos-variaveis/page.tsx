...
const custosIniciais = [
  { id: 1, descricao: "Matéria prima mês", valor: 4500, data: "2026-03-02", categoria: "Matéria prima" },
  { id: 2, descricao: "Comissão vendedores", valor: 2800, data: "2026-03-05", categoria: "Comissões" },
  { id: 3, descricao: "Google Ads", valor: 1200, data: "2026-03-10", categoria: "Marketing e vendas" },
  { id: 4, descricao: "Frete entregas", valor: 850, data: "2026-03-12", categoria: "Logística" },
];
...
export default function CustosVariaveis() {
  ...
  const maiorCusto = custos.reduce((max, c) => c.valor > max.valor ? c : max, custos[0]);
    ...
    <div className="min-h-screen flex" style={{background: "#020810"}}>
      <div className="w-64 min-h-screen flex flex-col" style={{background: "rgba(10,22,40,0.95)", borderRight: "1px solid rgba(59,111,212,0.15)"}}>
        ...
        <nav className="flex-1 p-4 space-y-1">
          {[
            ...
            { label: "Custos Variáveis", path: "/custos-variaveis", active: true },
      ...
      <div className="flex-1 p-8 overflow-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <h2 className="text-2xl font-bold" style={{color: "#c8d8f0"}}>Custos Variáveis</h2>
            </div>
            <p className="text-sm" style={{color: "#3a5a8a"}}>Gerencie seus custos variáveis do mês</p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            { label: "Total do Mês", value: `R$ ${totalMes.toLocaleString("pt-BR")}`, color: "#f87171" },
            { label: "Qtd de Lançamentos", value: `${custos.length} itens`, color: "#6ab0ff" },
            { label: "Maior Custo", value: maiorCusto ? `R$ ${maiorCusto.valor.toLocaleString("pt-BR")}` : "—", color: "#fbbf24" },
          ].map((card) => (
        ...
        <div className="flex gap-3 mb-6">
          <div className="flex items-center gap-2 flex-1 px-4 py-3 rounded-xl" style={{background: "rgba(10,22,40,0.8)", border: "1px solid rgba(59,111,212,0.15)"}}>
            ...
            <input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar custo variável..." className="bg-transparent flex-1 focus:outline-none text-sm" style={{color: "#c8d8f0"}}/>
          </div>
          ...
          <table className="w-full">
            <thead>
              <tr style={{borderBottom: "1px solid rgba(59,111,212,0.15)"}}>
                {["Descrição", "Categoria", "Data", "Valor", "% do Total", "Ações"].map(h => (
              ...
              {custosFiltrados.map((c, i) => (
                <tr key={c.id} style={{borderBottom: i < custosFiltrados.length - 1 ? "1px solid rgba(59,111,212,0.08)" : "none"}}>
                  ...
                  <td className="px-6 py-4">
                    <span className="text-xs px-3 py-1 rounded-full" style={{background: "rgba(251,191,36,0.1)", color: "#fbbf24"}}>{c.categoria}</span>
                  </td>
                  ...
                  <td className="px-6 py-4 text-sm" style={{color: "#6ab0ff"}}>{((c.valor / totalMes) * 100).toFixed(1)}%</td>
                  <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50" style={{background: "rgba(0,0,0,0.7)"}}>
          <div className="w-full max-w-md rounded-2xl p-8" style={{background: "#0a1628", border: "1px solid rgba(59,111,212,0.3)"}}>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold" style={{color: "#c8d8f0"}}>Novo Custo Variável</h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={novo.descricao} onChange={(e) => setNovo({...novo, descricao: e.target.value})} placeholder="Ex: Compra de matéria prima" className="w-full px-4 py-3 rounded-xl focus:outline-none text-sm" style={{background: "rgba(255,255,255,0.04)", border: "1px solid rgba(59,111,212,0.2)", color: "#c8d8f0"}}/>
              </div>
              <div>
