...
export default function Investimentos() {
  ...
  const exportarPDF = async () => {
    ...
    try {
      ...
      pdf.text(`${idioma === 'pt' ? 'Investimentos' : idioma === 'en' ? 'Investments' : 'Inversiones'} - ${new Date().toLocaleDateString(idioma === "en" ? "en-US" : idioma === "es" ? "es-ES" : "pt-BR")}`, pdfWidth - 14, 13, { align: "right" })
  ...
  const titulo = idioma === 'pt' ? 'Investimentos' : idioma === 'en' ? 'Investments' : 'Inversiones'
  const subtitulo = idioma === 'pt' ? 'Acompanhe sua carteira de investimentos' : idioma === 'en' ? 'Track your investment portfolio' : 'Sigue tu cartera de inversiones'
  const labelNovo = idioma === 'pt' ? 'Novo Investimento' : idioma === 'en' ? 'New Investment' : 'Nueva Inversión'
  ...
  return (
    ...
    >
      <div ref={conteudoRef}>
        {/* Card total */}
        <div className="rounded-2xl p-5 md:p-6 mb-6" style={{ background: "rgba(10,22,40,0.8)", border: "1px solid rgba(52,211,153,0.2)" }}>
          ...
          <p className="text-3xl md:text-4xl font-black" style={{ color: "#34d399" }}>{fmt(totalInvestido)}</p>
        </div>
        ...
        {investimentos.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {investimentos.map((inv) => (
              <div key={inv.id} className="rounded-2xl p-5 md:p-6" style={{
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <button onClick={() => excluir(inv.id)} className="flex-shrink-0">
