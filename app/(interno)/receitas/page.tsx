    ...
    <div className="relative rounded-2xl overflow-hidden" style={{
      ...
      <motion.div
        ...
        style={{
          ...
          boxShadow: `0 0 10px ${cor}, 0 0 20px ${cor}80`,
          borderRadius: "999px",
        }}
      />
      {/* Glow canto */}
      <div className="absolute top-0 left-0 w-20 h-20 pointer-events-none" style={{
        background: `radial-gradient(circle at top left, ${cor}12 0%, transparent 70%)`,
      }} />
      <div className="absolute bottom-0 right-0 w-20 h-20 pointer-events-none" style={{
        background: `radial-gradient(circle at bottom right, ${cor}08 0%, transparent 70%)`,
      }} />
...
export default function Receitas() {
  ...
  const abrirEdicao = (receita: Receita) => {
    setEditando(receita);
    setNovo({ descricao: receita.descricao, valor: String(receita.valor), data: receita.data, categoria: receita.categoria, status: receita.status });
    setModalAberto(true);
  };
        ...
        <div className="flex flex-col md:flex-row gap-3">
          <NeonBox cor="#3b6fd4">
            <div className="flex items-center gap-2 px-4 py-3">
              ...
              <input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder={t.receitas.buscar} className="bg-transparent flex-1 focus:outline-none text-sm" style={{ color: "#c8d8f0", minWidth: "200px" }} />
            </div>
          </NeonBox>
          <select value={filtroCategoria} onChange={(e) => setFiltroCategoria(e.target.value)} className="px-4 py-3 rounded-2xl focus:outline-none text-sm" style={{ background: "rgba(10,22,40,0.85)", border: "1px solid rgba(59,111,212,0.3)", color: "#c8d8f0" }}>
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={key}>
                    ...
                    <input
                      type={type}
                      value={novo[key as keyof typeof novo]}
                ...
                <motion.button
                  whileHover={{ scale: 1.02, boxShadow: "0 0 20px rgba(42,95,212,0.5)" }}
                  whileTap={{ scale: 0.98 }}
                  onClick={salvar}
                  disabled={salvando}
