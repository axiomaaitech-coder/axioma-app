...
export default function Receitas() {
  ...
  const textoShare = [
    ...
    `💰 ${t.receitas.totalReceitas}: R$ ${fBRL2(totalReceitas)}`,
    `📈 MRR: R$ ${fBRL2(mrr)} · ARR: R$ ${fBRL2(arr)}`,
    `📊 ${cx.crescimentoMoM}: ${cresc >= 0 ? "▲" : "▼"} ${Math.abs(cresc).toFixed(1)}%`,
    `_axiomaai.com.br_`,
  ].join("\n");

  const textoDetalhado = [
    `🚀 AXIOMA AI.TECH — ${t.receitas.titulo} (detalhado)`,
    ...receitasFiltradas.map((r) =>
      `${r.data ? new Date(r.data + "T00:00:00").toLocaleDateString("pt-BR") : "-"} | ${r.descricao} | ${r.categoria || "-"} | ${r.status === "recebido" ? "Recebido" : "Pendente"} | R$ ${fBRL2(r.valor)}`
    ),
    `_axiomaai.com.br_`,
  ].join("\n");
  ...
  const optPrev = optLinhaPrevisao(
    ...
    cx.realizado, cx.projetado, ct(CORES.verde), ct(CORES.cyan), temaClaro
  );
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0, y: 16 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 16 }} transition={{ duration: 0.22, ease: "easeOut" }} className="w-full max-w-md">
              <CanvasBox cor={ct("#6ab0ff")}>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="text-xs font-semibold tracking-wider uppercase mb-2 block" style={{ color: ct("#5a8fd4") }}>{t.receitas.status}</label>
