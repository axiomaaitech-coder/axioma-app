              ...
              <>
                ...
                {config.conferirGaveta && (
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-[12px] mb-3">
                      <button onClick={() => setCalculadoraAberta(true)}
                        className="h-[48px] flex items-center justify-center gap-2 rounded-xl text-[16px] font-black"
...
function CabecalhoModalAxioma({ titulo, lang, onFechar }: { titulo: string; lang: Idioma; onFechar: () => void }) {
  const { tokens } = useTemaPdv();
  return (
    <div className="flex items-start justify-between gap-3 -mx-[20px] -mt-[20px] mb-[16px] p-[16px]" style={{ borderBottom: `1px solid ${tokens.cardBorda}` }}>
      <div className="flex items-center gap-2 min-w-0">
        <Image src="/logo-aitech.png" alt="Axioma" width={32} height={32} className="object-contain shrink-0" />
        <div className="min-w-0">
          <h3 className="text-[20px] font-semibold truncate leading-tight" style={{ color: tokens.texto }}>{titulo}</h3>
          <p className="text-[12px] truncate leading-tight" style={{ color: tokens.textoMuted }}>{t("identidadeAxioma", lang)}</p>
        </div>
      </div>
      <button onClick={onFechar} className="shrink-0 p-1 rounded-lg" style={{ color: tokens.textoMuted }}><X size={20} /></button>
    </div>
  );
}
...
function ModalCalculadora({ lang, onUsar, onFechar }: { lang: Idioma; onUsar: (valor: number) => void; onFechar: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-start justify-center p-4 pt-[80px] overflow-y-auto" style={{ background: "rgba(2,8,16,0.6)" }}>
...
function LinhaDenominacao({ valor, qtd, onQtd, lang }: { valor: number; qtd: string; onQtd: (q: string) => void; lang: Idioma }) {
  ...
  return (
    <div className="h-[48px] flex items-center gap-3">
      <span className="text-[16px] font-semibold w-20 shrink-0" style={{ color: tokens.cardTexto }}>{moeda(valor)}</span>
      <input value={qtd} onChange={(e) => onQtd(e.target.value.replace(/[^0-9]/g, ""))}
        inputMode="numeric" placeholder="0" aria-label={t("qtdLabel", lang)}
        className="w-[80px] px-2 py-2 rounded-lg text-[16px] font-bold text-center outline-none shrink-0"
        style={{ background: tokens.inputBg, color: tokens.inputTexto, border: `1px solid ${tokens.inputBorda}` }} />
      <span className="text-[11px] font-bold uppercase shrink-0" style={{ color: tokens.cardTexto, opacity: 0.5 }}>{t("qtdLabel", lang)}</span>
      <span className="text-[16px] font-bold ml-auto text-right" style={{ color: tokens.cardTexto }}>{moeda(subtotal)}</span>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-start justify-center p-4 pt-[80px] overflow-y-auto" style={{ background: "rgba(2,8,16,0.6)" }}>
      <div className="w-full max-w-[480px] max-h-[85vh] overflow-y-auto rounded-2xl p-[20px]" style={{ background: tokens.modalBg, border: `1px solid ${tokens.acentoSuaveBorda}` }}>
        ...
        <div className="h-[80px] rounded-2xl mb-3 flex flex-col items-center justify-center" style={{ background: tokens.acaoBg }}>
          <p className="text-xs font-bold uppercase" style={{ color: tokens.acaoTexto, opacity: 0.85 }}>{t("totalGeral", lang)}</p>
          <p className="text-[32px] font-black leading-tight" style={{ color: tokens.acaoTexto }}>{moeda(total)}</p>
        </div>

        <p className="text-xs font-bold uppercase mb-2" style={{ color: tokens.cardTexto, opacity: 0.6 }}>{t("notasLabel", lang)}</p>
        ...
        <p className="text-xs font-bold uppercase mb-2" style={{ color: tokens.cardTexto, opacity: 0.6 }}>{t("moedasLabel", lang)}</p>
        <div className="flex flex-col gap-1 mb-3">
