...
**Verificação feita:** `tsc --noEmit` limpo, `next build` — `✓ Compiled successfully in 3.5min` — mesmo ponto de falha de sempre depois (`/api/pluggy/webhook`). **Pendente: Elias rodar `MEI-PRECOS-SALVOS-SQL.sql` no Supabase antes de testar salvar/editar/excluir preço** — sem a tabela, a calculadora e a Análise Axioma continuam funcionando normalmente (só a seção de salvar fica sem efeito).
