...
**Blindagem de identidade — nunca revelar Claude/Anthropic (rodada atual):** `app/api/ia-chat/route.ts` agora injeta no `system` prompt uma instrução fixa pra nunca se identificar como IA, Claude, Anthropic ou modelo de linguagem, mesmo se perguntado diretamente — a resposta deve sempre se apresentar como a inteligência/consultor do Axioma e redirecionar pra como pode ajudar. Mesma instrução adicionada aos prompts de `montarPromptCFO` (`lib/iaFinanceiraHelpers.ts`) e `montarPromptTributario` (`lib/iaTributariaHelpers.ts`). Em paralelo, varredura de textos de tela que expunham a implementação por trás (ex: "gerado por IA (Claude)... ativa quando a chave for ligada", "IA generativa", "IA indisponível") em Clientes, Contas a Receber, Fornecedores, Inadimplência, MEI (DAS, Faturamento, Reforma Tributária, IA Advisor) e nos textos-base (`cfoTextos.ts`, `meiTextos.ts`) — trocados por linguagem que não menciona o modelo por trás nem promete "ativar quando a chave ligar" (a chave já pode ligar a qualquer momento sem aviso ao usuário final). Nomes de módulo no menu ("IA Financeira", "IA Tributária", "IA MEI Advisor") foram mantidos — são nome de produto, não vazamento de implementação. Favicon do Axioma + `icons` em `app/layout.tsx` (favicon.ico + apple-touch) também entraram nesta rodada. `tsc --noEmit` limpo.
...
## 3-Z. Correção — tradução pendente do motivo da sugestão de destino + varredura de i18n no módulo (2026-07-25)

Elias achou, antes de testar: o tooltip que explica "por que sugeri esse destino" (seção 3-Y) só saía em português, e pediu varredura no módulo inteiro — não só o tooltip.

**Motivo da sugestão traduzido de verdade (não só o rótulo da tela):** os textos que os PARSERS geram (`lib/importarParsers.ts`) — "detectado como extrato bancário", "CNPJ emitente = sua empresa → venda", "coluna de vencimento → conta a pagar", etc. — eram string fixa em português dentro do motor de detecção, então nenhuma tela conseguiria mostrar isso em EN/ES nunca, por mais que a tela estivesse traduzida. Corrigido na raiz: `sugerirDestinoTransacao`, `parseOFX`, `parseXMLNFe`, `parseCSV`, `parseXLSX` e `parseArquivo` agora recebem o idioma ativo (`Lang = "pt"|"en"|"es"`, propagado desde a tela) e escolhem o motivo certo de um dicionário `MOTIVOS_DESTINO` com as 3 línguas completas — mesmo padrão de correção já aplicado no Centro de Custos (seção 3-R) quando o mesmo tipo de bug apareceu lá. Também traduzidas as palavras-chave de detecção (`PALAVRAS_RECEITA`/`PALAVRAS_CUSTO`, com lista própria por idioma — não é a mesma lista traduzida palavra por palavra, porque cognatos em espanhol não sempre bastam) e os textos auxiliares (descrição padrão de lançamento OFX sem memo, "Cliente"/"Fornecedor" genérico na NF-e).

**Varredura do módulo — achado real além do tooltip:** os rótulos de destino (Fluxo de Caixa, Receitas, Contas a Pagar...) e de status (Aguardando, Concluído...) eram constantes fixas em português (`DESTINOS`/`STATUS_INFO`, direto no `page.tsx`) usadas em ~15 pontos da tela — inclusive nos 4 dropdowns novos de distribuição automática de destino (seção 3-Y). Corrigido: os rótulos saíram das constantes e viraram `tt.destinoLabels`/`tt.statusLabels` (PT/EN/ES completos), com duas funções puras (`destinoLabel(tt, key)`, `statusLabel(tt, status)`) usadas em todos os pontos — dropdowns, texto de compartilhamento (WhatsApp/e-mail), PDF individual e PDF do painel geral. De caminho, mais alguns rótulos hardcoded nesses mesmos textos de compartilhamento/PDF ("Arquivo:", "Data Importação:", "Status:", cabeçalhos de coluna do PDF) também ganharam chave de tradução, por estarem dentro da mesma função já sendo corrigida.

**Fora do escopo desta rodada, registrado por honestidade:** o módulo tem outros textos fixos em português pré-existentes, de antes da funcionalidade de distribuição automática (toasts como "Template salvo!"/"Linha removida", confirmações como `window.confirm("Tem certeza...")`, título de PDF "Importações - ..."). Não fazem parte do que foi tocado nesta ou na rodada anterior — são um i18n de fundo que já existia no módulo. Se quiser, isso vira uma rodada própria de tradução (mesmo tratamento que Centro de Custos recebeu na sua vez, seção 3-R), separada desta correção.

**Arquivos alterados:** `lib/importarParsers.ts` (`Lang`, `MOTIVOS_DESTINO`, `PALAVRAS_RECEITA`/`PALAVRAS_CUSTO` por idioma, `DESCRICAO_FALLBACK_OFX`, `CONTRAPARTE_FALLBACK`, todas as funções de parse com parâmetro `lang`), `app/(interno)/importar-documentos/page.tsx` (`destinoLabel`/`statusLabel`, `tt.destinoLabels`/`tt.statusLabels`/novas chaves PT/EN/ES, `langAtual` propagado pro parser).
...
## 3-AB. Correção — 2 bugs achados na 2ª rodada de teste (destino trocado + regressão no extrato) (2026-07-25)

Elias confirmou extrato, vendas e leitura de "vencimento" corretos (seção 3-AA), mas achou 2 bugs novos — um no caso que tinha acabado de ser corrigido, outro uma regressão causada pela própria correção anterior. Mantido o princípio da seção 3-AA: regra mal feita, não "falta de IA" (ver `[[project_axioma_regra_vs_ia_importacao]]`).

**Bug A — destino trocado no arquivo de contas a pagar:** "Boleto fornecedor Papelaria" foi certo (Contas a Pagar), mas "Fatura energia eletrica" e "Salario funcionario" foram parar em Contas a RECEBER — errado, as duas são despesa que a empresa paga. Causa: dentro do bloco de vencimento (seção 3-AA), quando nenhuma palavra de `PALAVRAS_CUSTO`/`PALAVRAS_RECEITA` batia (energia/salário não estavam na lista), o código caía no sinal do valor, que por padrão é "entrada" pra qualquer arquivo com valor só positivo (típico de contas a pagar) — resultado: foi pra Contas a Receber por omissão. Corrigido em duas frentes: (1) `PALAVRAS_CUSTO` ganhou vocabulário real de despesa recorrente — salário, energia, água, aluguel, condomínio, imposto, tributo, despesa (+ equivalentes EN/ES); (2) o padrão do bloco de vencimento, quando NENHUMA palavra bate dos dois lados, deixou de cair no sinal do valor e virou Contas a Pagar direto (confiança baixa, motivo novo `vencimentoPagarPadrao`) — é o caso mais comum pra esse tipo de arquivo (despesa recorrente raramente tem palavra de custo explícita), Contas a Receber só quando há sinal claro de cliente/recebimento.

**Bug B — regressão no arquivo misto (virou tudo Fluxo de Caixa):** a correção do Bug 1 da seção 3-AA (`pareceExtrato`) tinha "boleto" no vocabulário de extrato — mas "Boleto fornecedor X" é uma conta a pagar comum, não extrato. Um arquivo misto com "Venda cliente Gama"/"Recebimento cliente Delta"/"Boleto fornecedor X" bateu a palavra "boleto" E tinha entrada+saída misturada (2º critério do `pareceExtrato` antigo), então o arquivo inteiro foi carimbado como extrato por engano, sobrescrevendo a classificação por linha que já estava certa. Corrigido: `PALAVRAS_EXTRATO` ficou só com vocabulário EXCLUSIVO de movimentação bancária (TED, DOC, PIX, tarifa, saque, "movimento"/"extrato" — removidos "boleto", "depósito", "transferência" isolada, genéricos demais e usados em documentos de negócio comuns) e o critério de "entrada+saída misturada" foi removido — agora só considera o arquivo um extrato quando o vocabulário bate em MAIORIA das linhas (>50%), critério único e mais rigoroso.

**Arquivo alterado:** `lib/importarParsers.ts` (`PALAVRAS_CUSTO` com vocabulário de despesa recorrente, `PALAVRAS_EXTRATO` restrito a vocabulário exclusivo de banco, `pareceExtrato` com critério único de maioria, bloco de vencimento em `sugerirDestinoTransacao` com padrão Contas a Pagar em vez de sinal do valor, motivo novo `vencimentoPagarPadrao` PT/EN/ES).
...
**Teto de leitura ainda é 5000 por tabela (Parte 6, seção 13, item 2), não virou RPC — dívida técnica registrada, adiada de propósito (decisão do Elias, 2026-07-26): sem cliente de alto volume ainda, o retorno de redesenhar agora é baixo perto do tamanho do redesenho (Causa Raiz e Radar de Oportunidades precisam de dado linha-a-linha, não de total agregado).** Retomar quando houver cliente de alto volume real. 5000 cobre qualquer volume de PME hoje.

**Arquivo alterado:** `app/(interno)/centros-custo/page.tsx` (`salvarLancamento`).

**Verificação feita:** `tsc --noEmit` limpo no projeto inteiro.

## 3-AF. Centro de Custos — 6 bugs do primeiro teste clicado na tela (2026-07-26)

Elias testou de verdade pela primeira vez (Simulador/Copiloto/Ações/Planilha renderizam certo) e reportou 6 problemas. Causa comum investigada: os bugs 1 e 2 têm a mesma raiz.

1. **Lápis de editar inerte na Planilha (causa raiz de vários bugs) — corrigido.** A célula só abria edição no DUPLO clique; o ícone de lápis (clique único, gesto natural) não fazia nada, e só aparecia na coluna Valor — nas outras colunas editáveis (inclusive Centro de Custo) não tinha nenhum indicativo visual. Corrigido em `CelulaEditavel` (`components/PlanilhaCentroCusto.tsx`): clique único abre a edição, ícone de lápis aparece em toda coluna editável.
2. **"Sem centro" nas análises (Insights/Oportunidades/Ranking/Cobertura) — não era bug separado, era consequência do #1.** Essas telas leem `lancamentosOrigem` direto (recarregado a cada salvamento) — assim que o lápis corrigido permitir atribuir centro num lançamento, as análises atualizam sozinhas, sem código adicional. Confirmado lendo o fluxo de dados, não precisou mudar nada aqui.
3. **Rateio — botões "Por headcount"/"Por área" ficavam mudos quando nenhum centro tem esse dado cadastrado.** `sugerirPercentuaisPorBase` retornava vazio e a tela não avisava nada — parecia bug, era dado faltando sem feedback. Corrigido: mostra aviso pedindo pra cadastrar headcount/área no centro ou usar "Igualmente". **"Igualmente" e "Aplicar Rateio" foram revisados a fundo e o código está correto (testado por leitura, não por clique — sem acesso a navegador nesta sessão)** — se continuar falhando depois desse fix, é sinal de algo que só aparece rodando no navegador (ver nota abaixo).
4. **Lançamentos manuais (`lancamentos_centro`) ficavam fora de duas análises.** Causa Raiz e Motor de Oportunidades continuam sem eles por design original (esses motores sempre foram escopados só pra Custos Variáveis/Contas a Pagar/Fornecedores, seção 3-Q) — não é bug. Mas achei 2 gaps reais e corrigidos: **Cobertura de Atribuição** (Score do módulo) não contava os manuais no total nem nos "sem centro"; e o **Simulador** não somava os lançamentos manuais tipo "custo" do período no custo variável baseline. Corrigidos em `page.tsx`.
5. **Copiloto não reconhecia "custo"/"gasto"/"despesa" como sinônimo de "caixa"** na pergunta "qual centro consome mais". Adicionados em `centroCustoInteligenciaHelpers.ts`.
6. **Mensagem de CPF/CNPJ inválido seca, sem tradução** — corrigida pra "CPF/CNPJ inválido — confira os dígitos" nas 3 línguas. Confirmado que já era possível salvar sem CNPJ (campo opcional, validação só roda se preenchido) — nada a mudar aí.

**Nota sobre o método:** todos os 6 itens foram corrigidos por leitura de código (sem clicar na tela — sem acesso a navegador nesta sessão). Os itens 1, 4, 5, 6 são bugs de lógica claros e a correção é direta. O item 3 (parte "Igualmente"/"Aplicar") não teve uma causa de código encontrada mesmo com revisão detalhada — se o teste do Elias mostrar que ainda falha, o próximo passo é investigar com navegador conectado (erro de runtime que só aparece na tela, não no código estático).

**Arquivos alterados:** `components/PlanilhaCentroCusto.tsx`, `lib/centroCustoInteligenciaHelpers.ts`, `app/(interno)/centros-custo/page.tsx`.

**Verificação feita:** `tsc --noEmit` limpo no projeto inteiro. Não testado clicando na tela.

## 3-AG. Centro de Custos — rateio sumindo da tela + edição na Planilha "duplicando" linha (2026-07-26)

Elias testou de novo e reportou 2 problemas, ambos com a mesma raiz por trás.

1. **Rateio (Ratear Custo) some depois de aplicado — não era bug de gravação, era falta de exibição.** Conferido: o rateio grava certo no banco e a Visão Geral já soma certo (o custo dividido entre os centros, sem duplicar o valor original). O problema é que a aba **Planilha** — o lugar que lista cada lançamento — nunca sabia que aquele lançamento tinha sido rateado: mostrava ele inteiro no centro original (ou "Sem centro"), sem nenhum sinal do rateio. Por isso "sumia": o usuário aplicava o rateio, voltava pra Planilha e via a mesma coisa de antes. **Corrigido:** a coluna "Centro de Custo" da Planilha agora mostra um selo roxo "Rateado" com a lista dos centros e o % de cada um, direto na linha do lançamento (passar o mouse mostra o detalhe completo). A linha continua com o valor cheio (não duplica), só o selo avisa que ele está dividido — igual já acontecia certo nos totais da Visão Geral.
2. **Editar pelo lápis na Planilha "duplicava" linha e recarregava a tela no meio — mesma causa raiz das duas coisas.** `carregarDados()` (a função que busca tudo de novo depois de qualquer salvamento no módulo) ligava `loading = true` toda vez, o que desmontava a tela inteira e mostrava a tela de carregamento — não só na Planilha, em QUALQUER salvamento do módulo (rateio, orçamento, lançamento manual, cadastro de centro). Ao voltar, a lista remonta do zero: agrupamento por centro reordena, scroll volta pro topo, e a linha editada pode reaparecer numa posição bem diferente (às vezes lá embaixo, num grupo novo) — o que parece "linha nova inserida", mas é a mesma linha, no mesmo lugar do banco, só reordenada visualmente. Não achei nenhum INSERT de verdade no caminho da edição (`atualizarCampoOrigem` sempre faz UPDATE por id) nem no salvamento manual (botão de salvar já ficava travado durante o envio, sem como duplicar por clique duplo). **Corrigido:** `carregarDados()` só mostra a tela de carregamento na primeira vez que a página abre; todo recarregamento depois de salvar (Planilha, rateio, lançamento, centro) agora é silencioso — atualiza os dados sem sumir com a tela, permitindo editar em sequência.
3. **Linhas duplicadas dos testes:** não encontrei sinal de duplicação real no código (todo salvamento é UPDATE por id ou INSERT protegido contra clique duplo) — o mais provável é que o que pareceu duplicado eram reordenações da tela recarregando (item 2). Não tive acesso a um navegador logado nem a uma chave de administrador do banco nesta sessão pra contar as linhas direto — se depois desse fix o Elias ainda ver alguma linha repetida de teste na Planilha, é só apagar direto por lá (lápis não serve pra excluir, mas dá pra apagar puxando o registro na tela de origem — Custos Fixos/Variáveis/Contas a Pagar/Fornecedores).
...
## 3-AL. MEI DAS & Obrigações elevado a padrão CFO com IA real (2026-08-01)

**2 bugs reais corrigidos** (não é achismo, é o mesmo padrão de honestidade técnica de sempre): (1) o vencimento do DAS estava fixo no dia 20, ignorando `mei_dados.dia_vencimento_das` (campo que "Configurar MEI" já salva) — quem configurou outro dia via prazo errado. (2) A detecção de atraso dependia do usuário clicar "Atrasado" manualmente — um mês nunca tocado na tela ficava escondido como "Pendente" pra sempre, mesmo com o vencimento já no passado. `competenciasDASDoAno` (novo em `meiHelpers.ts`) resolve os dois: monta a lista honesta de competências desde a abertura do MEI (ou desde janeiro) cruzando com `mei_obrigacoes`, e classifica atraso sempre por data — nunca por clique.

**Mapa de Consequências** (só aparece com atraso real, nunca alarme falso): linha do tempo horizontal com gradiente e marcador **alinhados aos marcos oficiais reais** (não uniformes) — vencimento → 61 dias (multa no teto de 20%) → 12 meses (CNPJ Inapto) → 24 meses (Dívida Ativa da União); dívida atualizada hoje (multa 0,33%/dia + juros Selic real do Banco Central, reaproveitando `calcularPenalidadeDASAtraso` já existente); projeção "bola de neve" 30/60/90 dias (`projecaoBolaDeNeveDAS`, novo); alerta de suspensão da contribuição do INSS. **Simulador de Parcelamento (PGMEI):** até 60 parcelas, mínimo R$50, trava automática (`maxParcelasDAS`), avisos educativos (1ª parcela ativa o acordo, 3 atrasadas cancelam), botão pro portal oficial — não emitimos guia. **Histórico do Ano:** reaproveita a mesma lista de competências que alimenta o cálculo da dívida (fonte única, sem duplicar). **Análise Executiva por IA real:** mesmo padrão funcional e **mesmo identificador de modelo (`'claude-sonnet-5'`)** confirmados funcionando hoje no IA MEI Advisor e no Faturamento MEI — não foi assumido, foi conferido nos dois arquivos antes de codar.

**Arquivos:** `lib/meiHelpers.ts` (`competenciasDASDoAno`, `calcularDividaDASAcumulada`, `projecaoBolaDeNeveDAS`, `faseRiscoDAS`, `maxParcelasDAS`, constantes `DIAS_MULTA_TETO`/`DIAS_CNPJ_INAPTO`/`DIAS_DIVIDA_ATIVA`/`PARCELA_MINIMA_DAS`/`PARCELAS_MAXIMAS_DAS` novos), `app/(interno)/mei/das/page.tsx`.
...
## 3-AO. MEI Precificação — favicon maior, calculadora de horas e "Meus Preços Salvos" (2026-08-02)

**Favicon aumentado:** `logo-aitech.png` (500×500) tinha o selo real ocupando só ~312×309 pixels no centro — quase 40% de margem transparente, por isso aparecia pequeno na aba. Recortado no contorno real do selo (bounding box do alpha, ~5% de respiro) e regerados a partir desse recorte: `app/favicon.ico` (16/32/48 num único arquivo, formato ICO nativo multi-resolução), `app/icon.png` (512×512), `public/apple-touch-icon.png` (novo, 180×180, fundo sólido — ícone da Apple não aceita transparência). `app/layout.tsx` atualizado pra apontar `apple` pro arquivo novo. Gerado com uma ferramenta de imagem local (não virou dependência do projeto — `package.json`/`package-lock.json` sem alteração). Cache de favicon é agressivo — só aparece após hard refresh (Ctrl+Shift+R) ou aba anônima.

**Calculadora de horas trabalhadas (diferencial):** no modo Por Hora/Por Projeto, em vez de exigir "horas por mês" de cabeça, o usuário informa **Horas por dia** + **Dias por semana** (+ opcional **% de horas produtivas**, desconta reunião/deslocamento) — `horasMensaisCalculadas()` (nova em `meiHelpers.ts`, horas/dia × dias/semana × 4,33 semanas/mês × %produtivo) calcula e mostra "Você trabalha ~X h/mês". O campo direto de horas/mês continua existindo como override — se o usuário editar ele diretamente, o cálculo automático para de sobrescrever (mesma lógica de "toque manual" já usada pro seletor de modo).

**"Meus Preços Salvos" — zero token:** tabela nova `mei_precos_salvos` (SQL em `MEI-PRECOS-SALVOS-SQL.sql`, `empresa_id NOT NULL`, RLS igual ao resto do projeto). Botão "Salvar Preço" grava nome + modo + todos os inputs/resultado num campo `dados` (jsonb) — só `insert`/`update`/`delete` direto no Supabase, nunca chama `/api/ia-chat`. Lista com lápis (reabre e preenche a calculadora pra editar, mesmo id vira UPDATE) e lixeira (exclui com confirmação nativa do navegador, mesmo padrão já usado em outras 5 telas do projeto). Paginação client-side de 10 em 10 sobre um teto de 200 registros carregados (mesmo padrão de teto usado no histórico do DRE/importações).
...
**Rodado por Elias em 2026-08-15 — confirmado**: view criada, política de produtos usando `empresas_do_usuario_operacional()` (Fase 0 ativa).

## 3-AV. PDV Fase 3, Etapa 1 — código da Frente de Caixa (carrinho em memória) + SQL das tabelas de venda, aguardando revisão (2026-08-15)

**Código entregue e testado por `tsc --noEmit` (limpo):**
- `lib/pdvHelpers.ts` — `listarProdutosPdv` agora recebe o papel de quem está logado; quando é operador, troca sozinha a fonte da consulta pra `vw_produtos_seguro` (Etapa 0). `segmento` virou opcional no filtro, pra a busca da Frente de Caixa poder procurar em todos os nichos de uma vez (a navegação do Catálogo continua sempre passando um segmento, comportamento dela inalterado).
- `app/(interno)/pdv/venda/page.tsx` (novo) — a Frente de Caixa: busca por nome/SKU/código de barras, bipagem que adiciona sozinha quando bate um código exato, carrinho com quantidade editável e total ao vivo. **"Finalizar Venda" fica desabilitado de propósito nesta etapa** — nada é gravado ainda, é só o carrinho em memória, como combinado.
- `app/(interno)/pdv/page.tsx` (Catálogo) — antes, o operador caía numa tela "em construção" sem saída. Agora é redirecionado sozinho pra `/pdv/venda` assim que a tela carrega. O dono/demais papéis ganharam um botão "Frente de Caixa" no topo do Catálogo pra chegar lá também (pra testar ou porque numa loja pequena o dono também vende no balcão).

**SQL entregue, NÃO RODADO — aguardando revisão do Elias:** `PDV-FASE3-ETAPA1-VENDAS-SQL.sql` (raiz do projeto) — cria `turno_caixa` (abertura de caixa com fundo de troco), `venda` (cabeçalho) e `item_venda` (cada linha vendida, com preço **e custo** congelados no momento da venda, pra medir margem real de uma venda antiga mesmo que o custo do produto já tenha mudado). Total da venda recalculado sempre pela soma real dos itens (nunca um número solto que pode dessincronizar). Multi-tenant nas 3 tabelas, mas usando a mesma função de sempre (`empresas_do_usuario()`) — não a variante que bloqueia o operador — porque abrir turno e vender é o trabalho dele. O corte da coluna de custo fica registrado como decisão pendente pra quando alguma tela vier a ler `item_venda` de volta (mesma lógica da view da Etapa 0). Detalhe de risco anotado no próprio arquivo: se `PDV-FASE0-EQUIPE-OPERADOR-SQL.sql` for rodado de novo no futuro, ele bloquearia essas 3 tabelas por engano — a lista de exceções dele precisaria ganhar essas 3 tabelas antes.
...
## 3-AY. Tesouraria — Rodadas 1 e 2 entregues, deploy corrigido (2026-09-03/04)

**Rodada 1 (Data Core, SQL só pra revisão, aplicado pelo Elias depois):** `CFO-TESOURARIA-RODADA1-SQL.txt` — 5 tabelas ativas (`tesouraria_config`, `tesouraria_conta`, `tesouraria_politica`, `tesouraria_cenario`, `tesouraria_alerta`) + 2 inertes preparadas pra Open Finance/histórico (`tesouraria_banco_metrica`, `tesouraria_forecast_precisao`) + 2 RPCs (`tesouraria_posicao`, `tesouraria_fluxo_projetado`). RLS `empresa_id IN (SELECT empresas_do_usuario())` em tudo, seed automático em toda empresa nova.

**Rodada 2 (engines + tela), commits `67985f8`→`0fb49ea`:** `lib/tesourariaHelpers.ts` (Posição de Caixa, Fluxo Projetado em 3 cenários — otimista/base/estressado, com fator de atraso real de AP/AR reaproveitado de `contasPagarHelpers.ts`, nunca inventado —, Liquidity Score 0-1000 sempre explicado, Idle Cash Detector, Treasury Radar com gravação+dedup de `tesouraria_alerta`), tela `/tesouraria` (Command Center) e `/tesouraria/config`, menu "Tesouraria" no grupo Financeiro do `TopNav.tsx`, rota protegida no `middleware.ts`.

**INCIDENTE — módulo inteiro invisível em produção, causa raiz encontrada e corrigida em 2026-09-04:** o Elias reportou que a Tesouraria não aparecia no app da Vercel depois de recarregar e procurar em todos os cards do menu. Investigação: `git branch -vv` mostrou os 4 commits da Rodada 2 marcados `ahead` — nunca tinham sido pushados pro `origin/main`, só existiam local. Nenhum deploy da Vercel chegou a rodar pra esse código (confirmado via API pública do GitHub, `commits/{sha}/status` — o commit da Rodada 1, esse sim pushado antes, tinha status `success`; os 4 da Rodada 2 não tinham status nenhum). Achado um segundo problema no caminho: o commit `67985f8` (engines) tinha sido criado ANTES de eu terminar `descreverAlerta()`/`tituloAlertaLocalizado()` no mesmo arquivo — o `tsc` local sempre deu limpo porque checava o arquivo em disco (já completo), não o snapshot commitado, que ficou incompleto; o commit `356969e` (Command Center) já importava essas funções, então o HISTÓRICO daquele commit isolado ficaria com o build quebrado. Corrigido com um commit novo (`b3beabb`, sem mudar comportamento, só fechando a lacuna) antes do push. Push feito, deploy da Vercel confirmado `success` via API (~2min). **Lição registrada:** daqui pra frente, depois de "commit + push", confirmar com `git branch -vv` (zero ahead/behind) antes de reportar a entrega como concluída — `tsc` limpo no working tree não é garantia de que o commit em si builda sozinho.
...
**Ajustes pontuais pós-entrega (2026-09-05, commit único):**
1. **Ações de status nas descobertas — conferido, já estava certo.** Toda descoberta no Control Room tem os 3 botões (Marcar como resolvida / Marcar como revisada / Ignorar) no modal de evidência, sem lápis nem lixeira em lugar nenhum — descoberta é leitura calculada, nunca cadastro do dono. Cada mudança de status grava em `contador_decisao_journal` (histórico fica). Conferido em código (`app/(interno)/contador/page.tsx`): zero import de ícone de editar/excluir, `aplicarAcao()` só aceita `'revisado' | 'resolvido' | 'ignorado'`.
2. **Mobile (~380px) nas 4 telas — conferido em código, sem quebra.** Grids em todas as 4 telas caem pra 1-2 colunas por padrão (Tailwind, antes do breakpoint `sm`) e só abrem mais colunas a partir de telas maiores. A única tabela do módulo (lista de descobertas) tem `minWidth` fixo mas vive dentro de um `overflow-x-auto` próprio — rola por dentro, nunca estoura a página. Colunas menos essenciais (Tipo/Impacto/Confiança) somem progressivamente em telas estreitas (`hidden sm:table-cell` / `md:` / `lg:`), sobrando sempre Prioridade + Descoberta + Status. Nenhum `width`/`minWidth` fixo fora desse único container. Não testado clicando na tela em 380px real — o `middleware.ts` corrigido na auditoria anterior agora barra `/contador` sem sessão (o que é o comportamento certo), e sem login real em chat (decisão consciente, seção 3-A) não dá pra ver a tela com dado carregado; fica pro Elias confirmar visualmente.

**O que ficou pra fases futuras (dependem de histórico acumulado e/ou IA real — Claude API segue desativada por decisão do Elias até ele ativar billing, ver seção sobre isso):**
- **Causal Engine completo** — a causa provável de hoje é "o maior lançamento individual da conta no mês" (1 fator). Causa multi-fator de verdade (decompor quanto veio de preço vs quantidade vs novo fornecedor vs sazonalidade) precisa de mais histórico e provavelmente de um modelo, não só SQL.
- **Digital Twin completo** — hoje "Se Eu Fizer Nada" é a linha de base do fluxo projetado (30/60/90/180d). Um gêmeo de verdade (simular a empresa inteira reagindo a uma mudança, não só caixa) depende do Causal Engine acima.
- **Counterfactual Engine** ("e se eu tivesse feito X em vez de Y") — precisa de histórico de decisões reais + resultado observado por tempo suficiente (o Decision Journal desta rodada é exatamente o que começa a acumular esse dado).
- **Anomaly Network** — a "anomalia correlacionada" de hoje cruza 2 sinais de UM fornecedor. Uma rede de verdade (correlacionar sinais entre VÁRIOS fornecedores/contas/clientes ao mesmo tempo) é outro nível de complexidade, também alimentado pelo que o Discovery Engine for acumulando.
- **ROI Engine** (medir o retorno real de uma decisão tomada a partir de uma recomendação) — mesma dependência do Counterfactual: precisa de tempo + Decision Journal maduro.
- **Regulatory Engine** (compliance/obrigações acessórias automatizadas) — fora do escopo desta camada de inteligência; é módulo próprio, não construído ainda.
- **ZIA / Accounting Autopilot** — gancho `contador_agente_execucao` já existe (Rodada 1, inerte). Ativar depende da Claude API real, hoje desligada.

**Verificação feita:** `tsc --noEmit` limpo a cada commit (5 commits desta entrega, todos verificados antes de commitar) e `next build` — `✓ Compiled successfully` (5min); o build só falhou depois disso, no mesmo ponto pré-existente e sem relação de sempre (`/api/pluggy/webhook`, falta chave local, Pluggy em modo teste). Conferido também que cada commit só importa função já exportada por um commit anterior (sem dependência futura). Testado no navegador SEM login real (mesma prática já registrada — seção 3-A): as 4 telas carregam sem crash e sem erro de console no estado "nenhuma empresa ativa"; teste de ponta a ponta com dado real fica pro Elias. `git branch -vv` confirmado zero ahead/behind depois do push, e o deploy da Vercel do commit final (`63ffcc2`) confirmado `success` via API do GitHub — mesma checagem registrada como lição na seção 3-AY.

**Achado na auditoria final, corrigido no mesmo commit de push (`f0e813c`):** `/contador` tinha ficado de fora da lista `isRotaProtegida` do `middleware.ts` — todo outro módulo financeiro está lá (é o que redireciona quem não está logado e aplica o paywall de `perfis.plano_ativo`). A RLS já protegia o dado, mas o gate de acesso do produto não estava sendo aplicado nessa rota. Corrigido.
...
## 3-BA. Fiscal (Axioma Fiscal Intelligence) — Rodadas 1 e 2 entregues (2026-09-05)

**O que é:** não é o documento inteiro de 62 seções planejado — é o núcleo que dá pra fazer com o dado atual: obrigações, descobertas (Fiscal Radar), Health Score e a Atividade Fiscal que faltava no cadastro. Copilot IA, Digital Twin fiscal, Regulatory Intelligence e ZIA ficam preparados/inertes. Deterministic-first absoluto — nenhum cálculo de imposto por IA em nenhuma linha.

**Rodada 1 (Data Core, SQL aplicado pelo Elias):** `FISCAL-RODADA1-SQL.txt`. Diagnóstico encontrou 2 sobreposições grandes com o que já existia e evitou duplicar em vez de simplesmente seguir o pedido original ao pé da letra:
- `fiscal_obrigacao` **não virou tabela nova** — `empresa_obrigacoes` já era o rastreador real de obrigações fiscais (DAS/DASN/DEFIS/DCTF/EFD/ECF/ECD, com geração automática por regime já pronta em `gerarObrigacoesPadrao`, `lib/empresaHelpers.ts`). Em vez de duplicar, ganhou só os campos que faltavam: `competencia`, `risco`, `evidencia`, `responsavel`, `origem`.
- `fiscal_config` **não virou tabela nova** — `empresas` já tinha `regime_tributario`/`cnae_principal`/`uf`/`cidade`. Só faltavam `atividade_fiscal` e `aliquota_iss_pct`, confirmados como inexistentes pelo próprio comentário do código (`lib/iaTributariaHelpers.ts:138-144`). Ganharam direto em `empresas`.
- Genuinamente novo (nenhum equivalente encontrado): `fiscal_descoberta` (mesmo desenho de `contador_descoberta`, tabela própria — fronteira de módulo), `fiscal_health` (Health Score, RLS SELECT-only), `fiscal_regra_versao` e `fiscal_agente_execucao` (inertes, ganchos pra Regulatory Intelligence e ZIA/Copilot fiscal). 2 RPCs SECURITY DEFINER: `fiscal_health_calcular`, `fiscal_obrigacoes_proximas`.

**Rodada 2 (Control Room + engines + telas), commits `8daaa9e`→`4a9a639`:**
- Menu "Fiscal" no grupo Contabilidade do `TopNav.tsx` + `/fiscal` na lista de rotas protegidas do `middleware.ts` — direto nesta rodada, sem repetir o esquecimento do `/contador` (seção 3-AZ).
- `lib/fiscalHelpers.ts` — Fiscal Health Score (lê `fiscal_health_calcular`, sempre explicado por `explicarFiscalHealth`, nunca aparece "pelado" na tela) e Fiscal Radar / Discovery Engine com 5 regras determinísticas: obrigação atrasada (estimativa de multa de mora 0,33%/dia teto 20%, Lei 9.430/96 art. 61), obrigação vencendo sem preparação, divergência entre o valor cadastrado e o imposto calculado pela regra do regime (usa `calcularImpostoRegime` com a atividade/ISS reais quando definidas), exposição fiscal crescente (último DAS vs. média dos 3 anteriores, mesma filosofia "último×média" do `contador_detectar_variacao_despesa`) e oportunidade de regime (`calcularEconomiaTributaria`, confiança `inferencia` — só sinaliza, nunca afirma). Reaproveita sem duplicar: Priority Engine (`pontuarDescoberta`) e tipos de descoberta vêm de `lib/contadorHelpers.ts`; todo cálculo de imposto/simulação de regime/alerta de reforma vem de `lib/iaTributariaHelpers.ts` (fonte canônica única) — nenhuma fórmula tributária reescrita.
- `/fiscal` (Control Room) — mesma filosofia do Contador: abre com status, não formulário. Health Score em destaque + explicado, 7 tiles (Crítico/Atenção/Pendências/Em Dia/Próx. Obrigações/Previsões/Descobertas), prévia das próximas obrigações, tabela de descobertas com Explain This (evidência completa, confiança fato/cálculo/inferência, ações Revisar/Resolver/Ignorar sem lápis/lixeira) e card de Reforma Tributária (`gerarAlertasReforma`, nenhuma alíquota nova inventada). Banner quando a atividade fiscal ainda não foi definida.
- `/fiscal/obrigacoes` (Calendário) — lê `fiscal_obrigacoes_proximas` com janela 7/30/60 dias, cor de risco calculada no client (nunca persistida, nunca dessincroniza do hoje real). Ação rápida marcar paga/dispensada reaproveita `atualizarObrigacao` (`lib/empresaHelpers.ts`) — mesma escrita, mesma auditoria já usada na aba Compliance de `/empresa`. Edição completa (nome, valor, recorrência) **não duplicada** — botão leva pra `/empresa`.
- `/fiscal/config` (Atividade Fiscal) — define atividade (Comércio/Indústria/Serviço/Misto) e alíquota de ISS, grava em `empresas`. Mostra lado a lado o imposto COM e SEM a atividade definida quando o regime atual usa esse dado (Lucro Presumido) — o "isso liga o cálculo correto" fica visível na tela, não só em comentário. Acesso dono/admin (mesmo padrão de `tesouraria/config`); `empresas` é owner-only por RLS (decisão já registrada em `MIGRACAO-MULTITENANT.sql`, não mexida aqui) — um admin não-dono que tentar salvar recebe o motivo real em vez de um "salvo" falso (anti-falha-silenciosa).

**O que ficou pra fases futuras (Copilot IA, Digital Twin fiscal, Regulatory monitoring, ZIA — dependem da Claude API real, hoje desativada por decisão do Elias até ele ativar billing):**
- **Copilot fiscal / ZIA** — gancho `fiscal_agente_execucao` já existe (Rodada 1, inerte).
- **Regulatory Intelligence** (monitoramento automático de mudança de regra/alíquota) — gancho `fiscal_regra_versao` já existe (Rodada 1, inerte), versionado (nunca sobrescreve).
- **Digital Twin fiscal completo** — hoje o Fiscal Radar detecta variação e diverge do que já aconteceu; simular cenários futuros de carga tributária é outro nível, mesma dependência do Causal Engine já registrado na seção 3-AZ.
- **Documentos fiscais (NF-e/Open Finance) alimentando `documentos_pendentes` do Health Score** — campo já existe no schema (fica 0 nesta rodada, evidence-first: sem fonte real ainda, não inventamos contagem), liga quando a integração de documentos for retomada (Importar Documentos segue pausado, seção 3-AC).

**Verificação feita:** `tsc --noEmit` limpo a cada um dos 5 commits desta rodada, verificado antes de cada commit. `npx eslint` limpo nos arquivos novos (1 warning de variável não usada, corrigido antes do commit). `next build` — `✓ Compiled successfully` (~5min); o build só falhou depois disso, no mesmo ponto pré-existente e sem relação de sempre (`/api/pluggy/webhook`, falta chave local, Pluggy em modo teste). Confirmado por chamada direta ao REST/RPC do Supabase (anon key, sem tocar dado de ninguém) que a Rodada 1 estava mesmo aplicada em produção antes de começar a Rodada 2 — todas as tabelas/colunas novas responderam `[]` (RLS filtrando, não erro de coluna inexistente) e as 2 RPCs devolveram `"Sem acesso a esta empresa"` (função existe e checa posse) em vez de erro de função não encontrada. Testado no navegador SEM login real (mesma prática já registrada — seção 3-A): `/fiscal` redireciona corretamente pro fluxo de login sem sessão (confirma que o middleware protege a rota); teste com dado real carregado fica pro Elias. `git log origin/main` confirmado com os 5 commits desta rodada depois do push (`4a9a639`).

**Confirmado na auditoria final:**
1. `empresa_id` (ou RLS equivalente) em toda leitura/escrita nova — conferido em `lib/fiscalHelpers.ts` linha a linha: todo `.from()`/`.rpc()` novo tem `.eq("empresa_id", ...)` explícito ou é protegido pela RLS `fiscal_descoberta_empresa`/`fiscal_health_select` (mesmo padrão já usado em `atualizarStatusDescoberta` do Contador, onde o `id` já é suficiente porque a RLS barra por empresa).
2. Fiscal Health Score sempre explicado — `explicarFiscalHealth()` é chamada toda vez que `health` existe na tela, nunca um número sozinho.
3. `empresa_obrigacoes` reaproveitado, não duplicado — confirmado na Rodada 1 (nenhuma tabela `fiscal_obrigacao` criada) e na Rodada 2 (`/fiscal/obrigacoes` lê e escreve na mesma tabela, edição completa continua em `/empresa`).
4. Config de atividade fiscal liga o cálculo correto de Lucro Presumido — visível na própria tela (`/fiscal/config` mostra o valor com e sem a atividade), não só no código.
5. Toda descoberta do Fiscal Radar tem evidência jsonb com valores reais (nunca `{}` vazio) — conferido nas 5 regras de `lib/fiscalHelpers.ts`.
6. Mobile — mesmo padrão responsivo já usado e testado no Contador (colunas ocultas progressivamente, tiles em grid 2/4/7, tabelas dentro de `overflow-x-auto` próprio); não clicado em 380px real pelo mesmo motivo da seção 3-AZ (sem login real em chat).
...
## 3-BC. Enterprise Design System — tema Claro completo em todo o Comercial, recuperação de sessão travada (2026-09-12)

**Contexto:** o sistema do Elias travou no meio da rodada de padronização de tema Claro/cores do tema Escuro nas 6 telas do Comercial (Clientes, Contas a Pagar, Contas a Receber, Estoque, Fornecedores, Inadimplência) + `lib/cfoCore.ts`. Ao retomar, o trabalho estava parcialmente aplicado e quebrado: `tsc --noEmit` acusava várias referências a variáveis que nunca chegaram a ser criadas (`CAMPO_BG`, `inputStyle`, `selectStyle`, `labelStyle` "não encontrados") nos formulários de Clientes, Estoque e Fornecedores — sinal de que o crash aconteceu no meio de uma refatoração de estilo de campo. Corrigido antes de qualquer coisa nova, com o mesmo padrão já usado nas demais telas (hook `useCampoEstilo`/`useCampoEstilos`/`useEstoqueTema` fora do componente, chamado também dentro do componente principal onde faltava).

**Lacunas reais encontradas e fechadas, além do que não compilava:**
- Contas a Pagar e Estoque tinham ficado sem o wrapper `<div data-theme={tema}>` que liga o CSS do tema Claro na tela inteira, e sem o botão de trocar tema (`<ThemeToggle />`) no topo — as duas telas mais visitadas do módulo ficariam presas no tema Escuro mesmo com o Claro ativado.
- Fundo escuro fixo sobrando em pontos que deveriam acompanhar o tema: cards do Radar de Risco/Consolidação em Fornecedores (~12 pontos), abas e dropdown de busca de produto no Estoque, e a telinha de carregamento (spinner) de Contas a Receber e Inadimplência — todos ficariam com uma caixa escura destacando sobre fundo branco no tema Claro.
- Gráficos que não repassavam a informação do tema pro motor de gráficos (`optBarrasH`, `optBarrasComparativo`, `optRosca` em 2 pontos do Estoque; `optRadar` de Fornecedores) — eixos e legendas ficariam ilegíveis (cinza claro sobre fundo branco) no tema Claro. `optRadar` (`lib/cfoCore.ts`) não tinha suporte a tema Claro nenhum ainda — adicionado agora, mesmo padrão dos outros gráficos (`optDispersao`, `optVelocimetro`, etc.).
- Limpeza de sobra: algumas variáveis de cor criadas pra essa conversão ficaram sem uso (`PAINEL_FUNDO_B`, `PAINEL_BORDA`, `CAMPO_BORDA`, `CAMPO_BG2` em Clientes; `PAINEL_BG`, `CAMPO_BG`, `CAMPO_BG2` em Contas a Pagar; `PAINEL_FUNDO`/`PAINEL_FUNDO_B` em Estoque) — removidas ou reaproveitadas em vez de deixar código morto.

**Verificação feita:** `tsc --noEmit` limpo. `next build` — `✓ Compiled successfully` (7min); falhou depois disso só na coleta de dados de `/api/produto/classificar-lote` por falta de chave do Supabase local, mesmo padrão de sempre (rota exige `service_role`, não tem no `.env.local` local). `eslint` rodado nos 7 arquivos: os avisos que sobraram (`cx` não usado em Clientes, ordem de `carregar` antes de declarado em Contas a Pagar/Contas a Receber/Inadimplência, `setState` dentro de `useEffect` em Estoque, `any` espalhado por `cfoCore.ts`) são todos de código já existente antes desta rodada, não deste commit — confirmado um a um comparando com o `git diff`, deixados de fora por não fazerem parte do pedido (tema Claro), não por displicência. Não testado no navegador com login real nesta sessão — mesma prática já registrada (sem credencial em chat). Com isso, **o menu Comercial completo (Clientes, Fornecedores, Contas a Pagar, Contas a Receber, Estoque, Inadimplência) está no mesmo padrão de tema Claro/Escuro dos módulos já entregues.**
...
Ao começar a Parte 6, identificado um buraco real: não existe nenhuma trava hoje que obrigue o usuário a ter empresa antes de usar os módulos — só a tela Empresa cria "Minha Empresa" automaticamente na primeira visita. Um usuário que nunca abriu essa tela e for direto pra Receitas (ou qualquer outro módulo) não teria `empresa_id` pra gravar.

**Decisão do Elias, 4 condições:**
1. **Ordem obrigatória de resolução** (em `obterEmpresaAtiva()`): (a) empresa própria do usuário → usa; (b) senão, vínculo existente em `empresa_usuarios` (convidado) → usa; (c) só se nenhuma das duas, cria "Minha Empresa" nova. **Isso é o que garante que, quando a tela de aceitar convite existir, um contador convidado logue e caia na empresa do cliente — não numa empresa vazia própria.** Se essa ordem não for respeitada, a tela de convite não funciona direito mesmo depois de construída.
2. **Atômico e idempotente** — criação feita dentro de uma função no banco (`obter_ou_criar_empresa_padrao()`, `SQL-EMPRESA-PADRAO.sql`) com trava (`pg_advisory_xact_lock` por usuário), não "select depois insert" no cliente. Duas gravações simultâneas nunca criam duas empresas.
3. **Marcada como incompleta** — coluna nova `empresas.cadastro_completo` (default `true` nas empresas existentes, `false` só nas criadas automaticamente). Aviso discreto e persistente no topo de toda tela interna (`components/TopNav.tsx`) enquanto `cadastro_completo = false`, nunca bloqueia. Some sozinho na primeira vez que o usuário salvar qualquer campo na tela Empresa (`atualizarEmpresa` agora sempre grava `cadastro_completo: true`).
4. **Criada no momento certo, não só na gravação** — caminho principal é `app/auth/callback/route.ts` (login por senha com confirmação de e-mail, ou Google OAuth), chamando a função assim que a sessão é criada. `obterEmpresaAtiva()` só cria como **rede de segurança** (usuário legado sem empresa, ou edge case que não passou pelo callback).

**Arquivos:** `SQL-EMPRESA-PADRAO.sql` (novo, aguardando Elias rodar), `lib/empresaHelpers.ts` (`obterEmpresaAtiva()` reescrita com a ordem a/b/c, `carregarEmpresaPorId()` novo), `app/auth/callback/route.ts` (chama a função nos dois fluxos de confirmação), `components/TopNav.tsx` (aviso discreto), `app/(interno)/empresa/page.tsx` (removida a criação antiga não-atômica, agora só lê via `obterEmpresaAtiva()`).

**Aguardando:** Elias rodar `SQL-EMPRESA-PADRAO.sql` no SQL Editor do Supabase (mesma janela do `MIGRACAO-MULTITENANT.sql` — sem isso, `obterEmpresaAtiva()` funciona pra quem já tem empresa, mas o caminho (c)/rede de segurança e o hook do login vão falhar silenciosamente até a função existir no banco).

## 13. PARTE 6 DA MIGRAÇÃO MULTI-TENANT — ajuste de código, COMPLETA (2026-07-23)

Depois do Elias rodar `MIGRACAO-MULTITENANT.sql`, toda gravação do app parou de aparecer (RLS passou a exigir `empresa_id`, código ainda não gravava). Esta rodada resolveu isso em 11 blocos, cada um fechado com `tsc --noEmit` limpo antes de passar pro próximo — sem pular checagem, como pedido.

**Blocos 1-10 — o padrão em todo módulo:** cada `page.tsx`/helper passou a resolver `empresaId` via `obterEmpresaAtiva()`, gravar `empresa_id` em todo `insert`/`upsert`, e trocar filtro de leitura manual (`.eq("user_id", ...)`) por filtro nenhum — quem decide o que aparece na tela agora é a política RLS por empresa, não mais o código. Isso é o que faz dono e (no futuro) convidado enxergarem os mesmos dados. Módulos: Empresa (sócios/documentos/auditoria/obrigações/equipe), Receitas, Custos Fixos, Custos Variáveis, Fluxo de Caixa, DRE, Endividamento, Metas, Investimentos, Precificação, Fornecedores (+ contatos/documentos/contratos/produtos/interações), Clientes, Contas a Receber, Inadimplência (+ `cobrancaHelpers.ts`), Centro de Custos (+ `centroCustoInteligenciaHelpers.ts`, ver achado abaixo), MEI (7 telas — só 2 gravam, as outras 5 só leem `mei_dados`/`receitas`), Importar Documentos (`importarHelpers.ts` — maior arquivo, 42 pontos), IA Financeira/IA Tributária (histórico de conversa + `carregarSnapshot`/`carregarDadosFiscais`, usados também pelo Dashboard).

**Achado real no meio do caminho — função de auditoria compartilhada sem `empresa_id`:** `registrarAuditoriaCentro` (em `lib/centroCustoHelpers.ts`) é chamada por Receitas, Custos Fixos, Custos Variáveis, Fornecedores e Centro de Custos — nenhum deles gravava `empresa_id` no rastro de auditoria. Corrigido uma vez só na função, não duplicado em cada chamador.

**MEI — troca de `onConflict`:** `mei/page.tsx` e `mei/das/page.tsx` faziam `upsert` em `mei_dados` com `onConflict: 'user_id'`; trocado pra `onConflict: 'empresa_id'` (o índice único já foi criado em `empresa_id` pela migração), com o payload passando a gravar `empresa_id`.

**RPC `centro_custo_totais` — escopo reduzido, não escondido.** Investigando o pedido, achado real: o teto de 300 lançamentos não alimenta só o total da tela — a Causa Raiz analisa cada lançamento individual (data/categoria/valor) e o Radar de Oportunidades também recebe os arrays crus, não uma soma. Um RPC que só devolvesse total-por-centro deixaria esses dois painéis lendo dado capado enquanto o resto da tela mostraria o número certo — dois números diferentes competindo na mesma tela, pior que hoje. Fazer certo exige redesenhar como a Causa Raiz/Oportunidades consomem dado, redesenho que não dá pra fazer com segurança sem testar no navegador (sem acesso nesta sessão) num módulo que o Elias ainda não testou clicando. Entregue por ora: o teto subiu de 300 pra 5000 por tabela (`lib/centroCustoHelpers.ts`) — resolve a perda silenciosa de dado pra qualquer volume real de PME, sem mexer na arquitetura. O RPC de verdade fica registrado como próximo passo dedicado (seção 4, item 4).

**Rotas de servidor — mais simples do que parecia.** Investigação mostrou que só uma das 3 rotas realmente não tem sessão de usuário:
- `app/api/pluggy/sync/route.ts` — **tem** sessão (chamada pelo próprio navegador do usuário, cookies presentes). Resolve `empresaId` com a mesma ordem dono→convidado, direto no arquivo (é uma rota, não vale extrair helper pra um só lugar).
- `app/api/pluggy/webhook/route.ts` — essa sim é chamada pela Pluggy direto (sem sessão, usa `service_role`). `empresa_id` agora vem junto do `user_id` já salvo na linha de `open_finance` (que passou a gravar `empresa_id` no momento da conexão, em `app/(interno)/open-finance/page.tsx`).
- `app/api/stripe/webhook/route.ts` — **nenhuma mudança precisou.** Só grava na tabela `perfis`, que por decisão do Elias (seção 11, Parte 4) continua com dono = usuário, nunca ganhou `empresa_id`.

**Verificação feita:** `tsc --noEmit` limpo no projeto inteiro, a cada um dos 11 blocos e no final. `next build` (produção) — `✓ Compiled successfully` — o build só falha depois disso, no mesmo ponto de sempre e sem relação (`/api/pluggy/webhook`, falta `SUPABASE_SERVICE_ROLE_KEY` no `.env.local` local — Pluggy segue em modo teste).

**O que NÃO foi verificado — não testado clicando na tela nesta sessão** (sem acesso a browser): criar/editar um registro real em nenhum módulo pra confirmar que `empresa_id` grava e aparece de volta; se algum módulo perdeu leitura de dado por engano na troca de filtro. Recomendo esse ser o primeiro teste antes de considerar a Parte 6 pronta pra uso real — criar um registro em 3-4 módulos diferentes (ex: Receitas, Metas, Fornecedores) e conferir que aparece.

**Arquivos alterados (41):** `lib/empresaHelpers.ts`, `lib/centroCustoHelpers.ts`, `lib/centroCustoInteligenciaHelpers.ts`, `lib/cobrancaHelpers.ts`, `lib/fornecedorHelpers.ts`, `lib/iaFinanceiraHelpers.ts`, `lib/iaTributariaHelpers.ts`, `lib/importarHelpers.ts`, `lib/inadimplenciaHelpers.ts`, `components/TopNav.tsx`, `components/PlanilhaCentroCusto.tsx`, `app/auth/callback/route.ts`, `app/api/pluggy/sync/route.ts`, `app/api/pluggy/webhook/route.ts`, e os `page.tsx` de: Empresa, Receitas, Custos Fixos, Custos Variáveis, Fluxo de Caixa, DRE, Endividamento, Metas, Investimentos, Precificação, Fornecedores, Clientes, Contas a Receber, Inadimplência, Centros de Custo, MEI (7 telas), Importar Documentos, IA Financeira, IA Tributária, Dashboard, Open Finance.
