...
- **Tesouraria** (`/tesouraria` + `/tesouraria/config`): **Command Center executivo, Rodadas 1 e 2 entregues (2026-09-03/04, ver STATUS-AXIOMA.md seção 3-AY).** Posição de Caixa direto do ledger (disponível/aplicado/restrito + livre de fato = disponível − reserva mínima, drill-down pro Razão), Fluxo Projetado em 3 cenários (otimista/base/estressado — desvio calculado do histórico real de atraso em AP/AR, nunca inventado), Liquidity Score 0-1000 sempre explicado em texto, Idle Cash Detector (só sinaliza, nunca recomenda investir), Treasury Radar (ruptura de caixa, concentração de banco/cliente, dívida alta, caixa ocioso — grava em `tesouraria_alerta` com dedup). Determinístico 100%, zero IA (ZIA é Rodada 3). Cor tema: azul (`#3b6fd4`, mesma do grupo Financeiro). Tabelas `tesouraria_*` (SQL em `CFO-TESOURARIA-RODADA1-SQL.txt`) + RPCs `tesouraria_posicao`/`tesouraria_fluxo_projetado`.
- **Contador** (`/contador` + `/contador/explicar` + `/contador/projecao` + `/contador/fechamento`): **CFO Intelligence, Rodadas 1 e 2 entregues (2026-09-04/05, ver STATUS-AXIOMA.md seção 3-AZ).** Não é contador automatizado nem 2º ledger — é a camada de INTELIGÊNCIA em cima do Accounting Core, Event Fabric, Tesouraria, Contas a Pagar e Estoque. Discovery Engine determinístico (`lib/contadorHelpers.ts`) grava descobertas com prioridade P0-P3 (Priority Engine: impacto×probabilidade×materialidade), causa, evidência e nível de confiança (fato/cálculo/inferência) — variação de despesa, concentração de fornecedor/banco/cliente, oportunidades (desconto perdido, capital parado em estoque, caixa ocioso), duplicidades, classificação suspeita, anomalia correlacionada. Control Room mostra status de inteligência (não balanço), Explain This Decision abre a evidência completa. "Explique Minha Empresa" (narrativa do ledger), "Se Eu Fizer Nada" (reaproveita o Gêmeo Financeiro da Tesouraria) e Continuous Close (`contador_close_readiness` + Data Trust Score). Determinístico 100%, zero IA (ZIA fica isolada pro futuro — tabela `contador_agente_execucao` já existe como gancho inerte). Cor tema: teal (mesma do grupo Contabilidade). Tabelas `contador_*` (SQL em `CONTADOR-RODADA1-SQL.txt`) + RPCs `contador_detectar_variacao_despesa`/`contador_close_readiness`.
...
- **Inadimplência** (`/inadimplencia`): **COMPLETO — 3 de 3 fases entregues (2026-07-22), módulo fechado.** Substitui o padrão CRUD antigo (tabela própria `inadimplencia`, órfã desde a Fase 1 — nunca apagada, só não é mais lida). Não recadastra contas nem clientes: lê `contas_receber`/`clientes` e reaproveita 100% o motor do Contas a Receber (`clienteIntelHelpers.ts`, `cobrancaHelpers.ts`) — zero tabela nova nas 3 fases, só colunas opcionais. **Fase 1:** Mapa Executivo de Risco, Dashboard 15 KPIs, Aging, Score de Risco Axioma. **Fase 2:** Central de Negociação, Régua de Recuperação Escalonada (5 estágios, sem disparo real), IA de Prevenção por regra, Painel de Alertas (12 tipos). **Fase 3:** Simulador Executivo de Recuperação (7 alavancas, 4 cenários), Previsão de Recuperação Multi-Horizonte (7-365 dias), Perda Esperada/Provisão PCLD com integração controlada na DRE, Custo de Cobrança × Valor Recuperável, Analytics (heatmap, curva ABC, evolução, distribuição, ranking), botão "Novo Caso" + editar/excluir título. Cor tema: índigo/safira profundo + platina. Arquivo próprio: `lib/inadimplenciaHelpers.ts`. `empresa_id` retrofitado corretamente na Parte 6 da migração multi-tenant (2026-07-23). Relatório completo em `STATUS-AXIOMA.md` seção 3-O ("RELATÓRIO FINAL — MÓDULO INADIMPLÊNCIA COMPLETO").
...
Centros de Custo, Planos, MEI (6 submódulos), Importar Documentos. Já em padrão CFO: IA Financeira, IA Tributária, Empresa, Relatórios, Fluxo de Caixa, DRE, Endividamento, Metas, Investimentos, Simulações, Precificação, Clientes, Fornecedores, Contas a Receber (COMPLETO, 3/3), Inadimplência (Fase 1 de 3).
