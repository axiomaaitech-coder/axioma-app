...
- **Alicerce:** `lib/cfoCore.ts`, `lib/cfoTextos.ts` — inclui agora o núcleo completo de DRE (`montarDRE`, `decomporVariacaoLucro`, `ponteLucroCaixa`, `semaforoSaude`, `runwayCritico`, `gerarConselhoCFO`, `projetarDRE`, gráfico waterfall `optCascata`).
...
- **DRE** (`/dre`): entregue nesta rodada. **Diagnóstico, não relatório estático.** Cascata completa com Análise Vertical/Horizontal por linha + waterfall, decomposição de causa raiz da variação do lucro, Ponte Lucro×Caixa (lucrativo no papel vs consumo de caixa real), Semáforo de Saúde + Runway até situação crítica, Conselho CFO acionável (ação + motivo + impacto em R$), projeção 3 meses, e **Histórico consultável** (tabela nova `dre_historico`, snapshot automático que congela ao fechar o período). Cor tema: verde/teal. Imposto calculado pelo regime tributário real da empresa (reaproveita `calcularImpostoRegime` da IA Tributária) — mesmo cálculo agora usado no Relatórios, que tinha o mesmo chute fixo antes.

## 🔜 PRÓXIMO NA FILA (ordem menu Financeiro)
**Endividamento** (`/endividamento`) — tabela `dividas` (descricao, tipo, valor_total, valor_pago, parcelas, vencimento, taxa_juros, user_id). Cor tema: rosa/magenta. Pesquisar brecha antes de codificar — candidatos: custo médio ponderado de capital, simulador de quitação antecipada, alerta de concentração de credores, cruzamento dívida/EBITDA (métrica clássica de risco ausente nos ERPs BR pra PME).
