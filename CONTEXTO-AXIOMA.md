...
- **Inadimplência** (`/inadimplencia`): **Fase 1 de 3 entregue nesta rodada — Centro de Inteligência de Recuperação Financeira.** Substitui o padrão CRUD antigo (tabela própria `inadimplencia`, que duplicava cliente/valor/vencimento). Não recadastra contas nem clientes: lê `contas_receber`/`clientes` e reaproveita 100% o motor do Contas a Receber (`clienteIntelHelpers.ts`, `cobrancaHelpers.ts`) — zero tabela nova, zero coluna nova. Entregue: Mapa Executivo de Risco (grade por cliente com valor devido, dias em atraso, nº de títulos, último pagamento, histórico de atrasos, probabilidade de recuperação/perda, Score Axioma, prioridade, impacto financeiro, responsável, status da negociação), Dashboard com 15 KPIs, Aging (0-30/31-60/61-90/90+) e Score de Risco Axioma (reaproveita o Score Axioma do Cliente já existente). Status da negociação lê `cobranca_compromissos` (já existente); novo compromisso registrado pela própria tela grava lá, referenciando `conta_id`/`cliente_id`. Cor tema: índigo/safira profundo + platina em acabamentos finos. Arquivo próprio: `lib/inadimplenciaHelpers.ts`. Relatório em `STATUS-AXIOMA.md` seção 3-M. Fases 2 e 3 aguardando aprovação do Elias.

## 🔜 PRÓXIMO NA FILA
Elias testar e aprovar a Fase 2 de Inadimplência → Fornecedores (já em padrão CFO) → E-commerce/PDV (alta prioridade — 2 clientes esperando). Perguntar ao Elias a ordem antes de começar. Ver `STATUS-AXIOMA.md` seção 3-M e 4.
