...
- **Inadimplência** (`/inadimplencia`): **Fase 2 de 3 entregue nesta rodada — Centro de Inteligência de Recuperação Financeira.** Substitui o padrão CRUD antigo (tabela própria `inadimplencia`, que duplicava cliente/valor/vencimento). Não recadastra contas nem clientes: lê `contas_receber`/`clientes` e reaproveita 100% o motor do Contas a Receber (`clienteIntelHelpers.ts`, `cobrancaHelpers.ts`) — zero tabela nova em ambas as fases, só colunas opcionais. **Fase 1:** Mapa Executivo de Risco (grade por cliente com valor devido, dias em atraso, nº de títulos, último pagamento, histórico de atrasos, probabilidade de recuperação/perda, Score Axioma, prioridade, impacto financeiro, responsável, status da negociação), Dashboard com 15 KPIs, Aging (0-30/31-60/61-90/90+) e Score de Risco Axioma. **Fase 2:** Central de Negociação (timeline unificada de `cobranca_interacoes`+`cobranca_compromissos`, parcelas/desconto/juros/multa/responsável, lápis pra editar), Régua de Recuperação Escalonada (Amigável→Formal→Protesto→Jurídico→Negativação, reaproveita `cobranca_regua_etapas` do Contas a Receber via coluna nova `estagio`, sem disparo real), IA de Prevenção por regra com recomendação de estratégia por cliente (desconto vs. parcelamento vs. jurídico, probabilidade estimada pelo Score Axioma) e Painel de Alertas (10 do Contas a Receber + 2 exclusivos: aumento de inadimplência, prazo excessivo). Cor tema: índigo/safira profundo + platina em acabamentos finos. Arquivo próprio: `lib/inadimplenciaHelpers.ts`. Relatório em `STATUS-AXIOMA.md` seções 3-M/3-N. Fase 3 aguardando aprovação do Elias.

## 🔜 PRÓXIMO NA FILA
Elias testar e aprovar a Fase 3 de Inadimplência → Fornecedores (já em padrão CFO) → E-commerce/PDV (alta prioridade — 2 clientes esperando). Perguntar ao Elias a ordem antes de começar. Ver `STATUS-AXIOMA.md` seção 3-N e 4.
