...
- **Contas a Receber** (`/contas-receber`): **Fase 1 de 3 entregue nesta rodada — Central de Inteligência Financeira de Recebimentos.** Reaproveita 100% o motor de `lib/clienteIntelHelpers.ts` (Clientes) — `montarSnapshotsCarteira` já calculado, sem duplicar. 3 blocos novos no mesmo arquivo: **Score Axioma do Cliente** (0-1000, mesmo motor de `calcularScoreAxiomaFornecedor` de Fornecedores Fase 3 — 12 critérios pedidos, 3 sem tabela ainda ficam "sem dados": disputas/cancelamentos/renegociações), **KPIs executivos** (17, incl. DSO real medido — não fórmula chutada — e índices de inadimplência/pontualidade), **Aging de carteira** (4 faixas, gráfico ECharts). Dashboard Executivo com drill-down real, modal de cadastro corrigido com `createPortal` (mesmo fix real do bug de Clientes v3, não a solução paliativa). Cor tema: esmeralda/teal profundo + dourado champagne em detalhes finos. Tabela `contas_receber` — 3 colunas novas pedidas (`responsavel`, `prioridade`, `projeto`) ainda não existem, SQL em `STATUS-AXIOMA.md` seção 3-J; até rodar, o resto do módulo funciona normalmente (fallback gracioso no save, não bloqueia o cadastro). Fases 2 e 3 aguardando Elias detalhar escopo e aprovar.

## 🔜 PRÓXIMO NA FILA
Fornecedores (já em padrão CFO) e Inadimplência ainda no padrão CRUD antigo → E-commerce/PDV (alta prioridade — 2 clientes esperando). Perguntar ao Elias a ordem antes de começar. Ver `STATUS-AXIOMA.md` seção 3-J e 4.
