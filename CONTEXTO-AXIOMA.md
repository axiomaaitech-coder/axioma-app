...
- **Contas a Receber** (`/contas-receber`): **Fase 1 (Central + Dashboard + Aging + Score) e Fase 2 (Cobrança Inteligente + IA Explicativa + Alertas) entregues, de 3.** Reaproveita 100% `lib/clienteIntelHelpers.ts` (Clientes) — `montarSnapshotsCarteira`, sem duplicar. Fase 1: **Score Axioma do Cliente** (0-1000, mesmo motor do Score Corporativo de Fornecedores Fase 3), **17 KPIs executivos** (incl. DSO real medido), **Aging** (4 faixas, ECharts). Fase 2 (`lib/cobrancaHelpers.ts`, arquivo próprio): histórico de contato/negociação e promessas/acordos por conta, **fila de cobrança priorizada** pelo Score Axioma, **régua de cobrança configurável** (dias relativos ao vencimento + canal + mensagem-modelo — sem disparo real ainda, só organiza), **10 tipos de alerta preditivo** (destaque: "cliente começando a atrasar" antes de virar inadimplência), e **IA Financeira Explicativa por regra** (O que aconteceu/Por quê/Impacto/Ação + **probabilidade de recebimento %** por conta, reaproveitando `scoreRecebimento` da Fase 1). Modal corrigido com `createPortal`. Cor tema: esmeralda/teal profundo + dourado champagne em detalhes finos. Tabela `contas_receber` + 3 tabelas novas da Fase 2 (`cobranca_interacoes`, `cobranca_compromissos`, `cobranca_regua_etapas`) — SQL em `STATUS-AXIOMA.md` seções 3-J e 3-K; até rodar, cada parte funciona normalmente com fallback gracioso. Fase 3 aguardando Elias detalhar escopo e aprovar.

## 🔜 PRÓXIMO NA FILA
Fornecedores (já em padrão CFO) e Inadimplência ainda no padrão CRUD antigo → E-commerce/PDV (alta prioridade — 2 clientes esperando). Perguntar ao Elias a ordem antes de começar. Ver `STATUS-AXIOMA.md` seção 3-K e 4.
